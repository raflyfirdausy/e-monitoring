                ...
                <div class="col-lg-12"> 
                  <label>UPT</label> 
                   <select class="form-control select2" style="width: 100%;" name="kode" id="kode">
                    ...
                    <?php $query = mysqli_query($conn,"select * from m_upt");
                          while ($data = mysqli_fetch_array($query)) {?>
                    <option value="<?=$data['id']?>"><?=$data['nama_upt']?></option>
                    <?php }?>
                  </select>
                </div>
              </div> 
              ...
              <div class="box-body">  
                <table id="example1" class="table table-bordered table-striped">
                    <thead>
                        <tr class="headings">
                            <th width="3%">No </th>
                            <th width="15%">Nama UPT </th>
                            <th width="10%">Nama UTLG</th> 
                            <th width="10%">Aksi</th>
                        </tr>  
                    </thead>
                    <tbody> 
                        <?php 
                            $query = mysqli_query($conn,"select * from m_utlg");
                            $no = 1;
                            while ($data = mysqli_fetch_array($query)) {?>
                                <tr>
                                    <td><?php echo $no; ?></td>
                                    <td><?=@$data['id_upt']; ?></td>
                                    <td><?php echo $data['nama_utlg']; ?></td>
                                    <td>  
